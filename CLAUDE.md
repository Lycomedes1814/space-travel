...
- **Trash behavior**: delegate to platform trash tools (`gio trash`, then `trash-put`); if neither is installed, trashing is unavailable.
...
4. Trash helpers (`run_trash_command`, `do_trash`)
...
| `d` | Move selected entry to the system trash via `gio trash` or `trash-put` (confirms with y/n) |
| `q` | Quit |
